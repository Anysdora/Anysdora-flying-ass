...
//acc_gyro == 1, 表示上电后acc自动校正
Key g_MatrixKey = {0, 0, 0, 0, 0, 1};

u8  TX_ADDRESS[TX_ADR_WIDTH]= {0x12,0x23,0x34,0x45,0x56};	//本地地址
u8  RX_ADDRESS[RX_ADR_WIDTH]= {0x12,0x23,0x34,0x45,0x56};	//接收地址
...
//	NRF24L01_Write_Reg(NRF_WRITE_REG+RF_SETUP,0x07);	//设置TX发射参数,0db增益,1Mbps,低噪声增益开启
/****************************************************************************************************************/
	if(model==1)			//RX
	{
...
//	NRF24L01 IRQ引脚中断配置
//	MiniSTM32 	PC5
void EXTI_NVIC_NRF24L01_Init(void)
{               
//	引脚配置
...
//	中断配置
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO,ENABLE);				//IO口中断复用时钟开启	
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOC,GPIO_PinSource3);		//设置IO口与中断线映射
	
	EXTI_InitStructue.EXTI_Line=EXTI_Line3;					//设置外部中断线
	EXTI_InitStructue.EXTI_Mode=EXTI_Mode_Interrupt;		//中断请求	
	EXTI_InitStructue.EXTI_Trigger=EXTI_Trigger_Falling;	//上升沿触发
	EXTI_InitStructue.EXTI_LineCmd=ENABLE;					//中断线使能
