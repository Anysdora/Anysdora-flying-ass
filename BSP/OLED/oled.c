...
void OLED_Init(void)        
{
  GPIO_InitTypeDef OLED_GPIO_InitStructure;
	...
	RCC_APB2PeriphClockCmd(OLED_RCCEN, ENABLE); 	 //GPIO濮嬬粓浣胯兘
  ...
  OLED_GPIO_InitStructure.GPIO_Pin = OLED_SCL|OLED_SDA|OLED_RST|OLED_DC;
	...
	GPIO_Init(OLED_GPIO, &OLED_GPIO_InitStructure); 		           //閰嶇疆OLED绔彛寮曡剼涓篏PIO杈撳嚭
...
{ 	
	...
	for(y=0;y<8;y++)
	{
		OLED_Set_Pos(28,y);		   //坐标设置函数 x列（0-127 ) y行（0-7）
